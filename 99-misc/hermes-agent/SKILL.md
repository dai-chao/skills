...
When the user is switching machines or wants a backup of their Hermes state, export the memory core and guide them through migration.

### What to export
- `~/.hermes/memories/MEMORY.md` — agent notes about environment, conventions, tool quirks.
- `~/.hermes/memories/USER.md` — user profile (preferences, role, style).
- `~/.hermes/SOUL.md` — optional persona file.
- `~/.hermes/config.yaml` — model, tool, and provider settings.
- `~/.hermes/skills/` — custom skills.
- `~/.hermes/cron/` — scheduled jobs.

### Steps
1. Read `MEMORY.md` and `USER.md` (and `SOUL.md` if present).
2. Create a self-contained Markdown export on the user's desktop named `hermes-memory-core.md`.
3. Include the memory content and a migration guide.
4. Remind the user that credentials (API keys, tokens) live in `~/.config/`, `.env`, or the keychain and must be migrated separately.

### Pitfalls
- Do NOT copy sensitive credentials into the export unless explicitly requested.
- Do NOT copy `.lock` runtime files.
- Adjust paths if the target machine uses a different home directory path.

See `references/memory-migration-export-template.md` for the export file layout.

## Third-Party Agent Skill Marketplaces

Some third-party platforms (e.g., 才虫, 43Chat) require a fixed onboarding flow: install skill files → register the agent → a human claims the agent → save a complete API key → configure a heartbeat.

### Standard workflow
1. Fetch the platform's `SKILL.md` and `skill.json` (usually via curl or `web_extract`).
2. Install all required files under `~/.hermes/skills/<skill-name>/` while preserving official filenames (case-sensitive).
3. Run installation self-checks: file list, JSON validity, version match, credential directory writable, and an unauthenticated probe that returns 401.
4. Register the agent. Confirm nickname and bio with the user; if the user authorizes, fill them in and report what was used. If the nickname is taken, **do not** auto-append numbers — ask the user.
5. Save the **complete** API key returned by registration. **Do not** save a masked or truncated key (e.g., `73ed...f8e7` or `***`). If the output is masked, ask the user for the full key or guide them to the official `reset-api-key` flow via SMS.
6. Verify immediately with `agent.me` (or the platform equivalent). A 401 means the key is wrong; fix it before any write operation.
7. Send the user the `claimUrl` and ask them to complete phone verification themselves. Do not complete verification for them.
8. After the user confirms the claim, update the credential file (`claimed: true`) and verify again.
9. Configure the heartbeat per the platform's documentation, using a state file and read-only polling. Do not write to the platform during heartbeat.
10. Explain the skill's triggers, behavior rules, and run a one-time market browse (`explore_task.list` or equivalent).

### Key pitfalls
- Saving a truncated or masked API key causes every subsequent authenticated call to fail.
- Calling write APIs repeatedly with a bad key wastes time and may trigger rate limits. A 401 means stop and fix the key first.
- Omitting a `lastHeartbeatCheck` timestamp or miscomputing intervals causes missed or duplicate heartbeats.
- Do not mark `claimed: true` until the user explicitly says they finished the phone verification.

See `references/third-party-skill-onboarding-caichong.md` for a concrete registration log and failure modes.
