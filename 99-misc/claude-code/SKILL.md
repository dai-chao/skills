...
| `ANTHROPIC_BASE_URL` | Override the default API endpoint (e.g., for third-party proxies like OpenRouter, yunwu.ai) |
| `ANTHROPIC_MODEL` | Override the default model (used when the proxy requires a specific model name) |
...
Claude Code can route through third-party API proxies (e.g., OpenRouter, yunwu.ai/云雾平台) instead of calling Anthropic directly. This is configured via `settings.json` or environment variables.
...
```json
{
  "env": {
    ...
    "ANTHROPIC_BASE_URL": "https://yunwu.ai/v1/chat/completions",
    "ANTHROPIC_MODEL": "claude-opus-4-8:stable",
...
**Key fields:**
- `ANTHROPIC_AUTH_TOKEN` — Your API key from the proxy platform (not Anthropic's key)
- `ANTHROPIC_BASE_URL` — The proxy's chat completions endpoint (must end in `/v1/chat/completions` for OpenAI-compatible proxies)
- `ANTHROPIC_MODEL` — The model name as recognized by the proxy (may include routing suffixes like `:stable`, `:floor`, `:nitro`)
- `model` (top-level) — Must match `ANTHROPIC_MODEL` for Claude Code's model picker

### Model Name Pitfalls

Proxies often use model names that differ from Anthropic's official names:

| Proxy | Example Model Name | Notes |
|-------|-------------------|-------|
| yunwu.ai | `claude-opus-4-8:stable` | Requires `:stable`/`:floor`/`:nitro` suffix for routing |
| OpenRouter | `anthropic/claude-3.5-sonnet` | Provider-prefixed |
| Official Anthropic | `claude-3-5-sonnet-20241022` | Full dated model ID |

**If you get "model may not exist or you may not have access":**
1. Check the proxy's console for the exact model name they expose
2. Try with and without routing suffixes
3. Ensure both `ANTHROPIC_MODEL` (env) and `model` (top-level settings) are set and match
4. Verify your proxy account has quota/credits for that model

### Per-Project vs Global Proxy Config

- **Global** (`~/.claude/settings.json`) — All projects use the proxy
- **Project** (`.claude/settings.json`) — Only this project uses the proxy
- **Local** (`.claude/settings.local.json`) — Personal override, gitignored

To temporarily bypass the proxy for a single command, use `--bare` mode with explicit env:
```
ANTHROPIC_API_KEY=your-real-key ANTHROPIC_BASE_URL= claude -p "task" --bare
```
