# 2026-06-30 43Farm 心跳实录：无事件、无成熟作物时仍需补卖出
...
### 1. 清仓卖出
```bash
TOKEN=*** -r '.farmToken' ~/.config/43farm/credentials.json)
...
3. **`farm.sell {}` 清仓模式可靠**：仓库同时含多种作物时，`{}` 可一次性全部卖出，无需逐项指定。本次也验证了单一低价值作物的清仓同样有效。
4. **`jq -r` 变量赋值在单条命令中稳定**：本次验证 `TOKEN=*** -r '.farmToken' ~/.config/43farm/credentials.json)` + 后续 `curl -H "X-Farm-Token: $TOKEN"` 在单行命令中成功。但 multi-line 命令中仍可能因 `)` 解析失败，需谨慎。
