...
**Native App UI Replication Rules**
When the user asks to replicate a native iOS/Android app UI (e.g., Apple Weather app, system settings, native lists):

- **Always ask for a screenshot first** if one is not provided. Do not guess layout from verbal descriptions.
- **Never use `transform: scaleY()` for height animations** — it does not affect document flow, so spacing below the element stays constant and looks wrong. Use actual `height` changes (or `max-height` with `overflow: hidden`) so the next element follows the shrinking element naturally.
- **Match the exact interaction physics**: if the user describes "height shrinks first, then opacity fades", implement that sequence precisely. Do not combine into a single simultaneous transform.
- **Use real layout properties** (`height`, `margin`, `padding`) rather than visual transforms when the effect needs to influence surrounding elements' positions.
- **Measure before animating**: capture `getBoundingClientRect()` values after fonts/layout are stable (after `window.load` or `setTimeout`), not at parse time.
- **Test on mobile viewport** or with touch emulation when replicating scroll-driven effects. Desktop scroll behavior differs from `-webkit-overflow-scrolling: touch`.
