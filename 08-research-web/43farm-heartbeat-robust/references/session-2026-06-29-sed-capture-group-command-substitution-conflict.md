...
/bin/bash: eval: line 17: `FARM_TOKEN=$(cat "$HOME/.config/43farm/credentials.json" | sed -n 's/.*"farmToken"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p')'
```
...
- `43farm/references/cron-shell-quoting-pitfalls.md` — 更全面的 shell 引号陷阱汇总（含本模式）
...
- `references/session-2026-06-29-jq-variable-assignment-reliable.md` — `TOKEN=$(jq -r ...)` 单变量赋值模式的成功案例
